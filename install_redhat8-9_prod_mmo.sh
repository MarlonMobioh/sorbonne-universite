...
# - Renseignement dans fichier de log mmo-scriptfirstboot.log
...
read ip_address
echo -e "\e[91mEntrez le masque de sous-réseau (ex: 24 pour 255.255.255.0) : \e[0m"
read subnet_mask
...
# Demander à l'utilisateur s'il y a de nouveaux comptes à créer
...
if [[ "$response" == "oui" ]]; then
    while true; do
        ...
        if ! user_in_wheel_group "$new_username"; then
            usermod -aG wheel "$new_username"
            echo -e "\e[91mUtilisateur $new_username ajouté au groupe wheel.\e[0m"
        else
            echo -e "\e[91mUtilisateur $new_username est déjà dans le groupe wheel. Ignorer l'ajout.\e[0m"
        fi
...
timedatectl
sleep 3
echo -e "\e[92mConfiguration de chronyd avec l'adresse IP 134.157.23.254 (r-v46.reseau.jussieu.fr) effectuée.\e[0m"
...
echo -e "\e[94m*** Abonnement Redhat 8a85f99977b0c0420177f2a086211111 attaché ***\e[92m"
sleep 3
